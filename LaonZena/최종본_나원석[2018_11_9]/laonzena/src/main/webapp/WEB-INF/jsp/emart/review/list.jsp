...
<body>
		...
		<tbody>
			...
			<c:forEach var="emart" items="${list}">
				...
				<tr>
					...
					<td>
						<c:if test="${emart.serName==null}">
							<img class="product" src='<c:url value="/upload/default.png"/>'/>	
						</c:if>
						<c:if test="${emart.serName!=null }">
							<img class="product" src='<c:url value="${emart.serPath}/${emart.serName}"/>'/>
						</c:if>
					</td>
					<td>${emart.product}</td>
    ...
    $(document).on("click","#submit", function () {
    	...
    	if($.trim($(".note-editable").text())=="") {alert("내용을 입력 해 주세요"); return false;};
//     	if($("#boardContent").val()=="") {alert("내용을 입력 해 주세요"); return false;};
